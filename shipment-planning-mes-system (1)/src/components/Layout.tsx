...
const navItems = [
  ...
  { path: '/erp-pool', label: 'ERP Havuzu', icon: Database },
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-slate-900 flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 ${
        ...
        <div className="px-4 py-3 border-b border-slate-700">
          <div className="bg-slate-800 rounded-lg p-2.5">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={triggerSync} disabled={syncStatus.syncing} className="text-blue-400 hover:text-blue-300 transition-colors">
            ...
            {!syncStatus.syncing && (
              <div className="mt-1.5">
                ...
                <div className="h-1 bg-slate-700 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${(syncCountdown / 60) * 100}%` }} />
                </div>
              </div>
            )}
          </div>
          ...
          {currentUser?.role === 'admin' && (
            <>
              <div className="pt-4 pb-2">
                <div className="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Yönetim</div>
              </div>
              {adminNavItems.map((item) => {
                ...
                return (
                  <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group ${
                      active ? 'bg-red-600 text-white shadow-lg shadow-red-900/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'
                    }`}>
                    <Icon size={17} />
                    <span className="flex-1">{item.label}</span>
                    {active && <ChevronRight size={14} className="opacity-60" />}
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p className="text-white text-sm font-medium truncate">{currentUser?.fullName ?? currentUser?.username ?? 'Kullanıcı'}</p>
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-800 text-sm transition-colors">
            <LogOut size={14} />
            <span>Çıkış Yap</span>
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="flex items-center gap-3 px-4 py-3 bg-white border-b border-slate-200 shadow-sm z-10">
          ...
          <div className="relative">
            <button onClick={() => setNotifOpen(!notifOpen)} className="relative p-2 rounded-xl text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors">
              <Bell size={20} />
              {unreadCount > 0 && (
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                  ...
                  <button onClick={clearNotifications} className="text-slate-400 hover:text-slate-600 text-xs">Temizle</button>
                </div>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.slice(0, 20).map((n) => (
                      <div key={n.id} onClick={() => markNotificationRead(n.id)}
                        ...
                        <div className="min-w-0">
                          <p className={`text-sm leading-snug ${!n.read ? 'text-slate-800 font-medium' : 'text-slate-600'}`}>{n.message}</p>
